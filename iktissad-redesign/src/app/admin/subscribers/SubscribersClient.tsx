...
function relativeDate(
  ...
  if (diffDays < 30) return labels.daysAgo.replace('{count}', diffDays.toLocaleString('ar-SA-u-ca-gregory-nu-latn'));
  const diffMonths = Math.floor(diffDays / 30);
  if (diffMonths < 12) return labels.monthsAgo.replace('{count}', diffMonths.toLocaleString('ar-SA-u-ca-gregory-nu-latn'));
  const diffYears = Math.floor(diffMonths / 12);
  return labels.yearsAgo.replace('{count}', diffYears.toLocaleString('ar-SA-u-ca-gregory-nu-latn'));
}
...
export default function SubscribersClient() {
    ...
    const csv = Papa.unparse(
      toExport.map((s) => {
        ...
        return {
          ...
          'تاريخ الانضمام': new Date(s.createdAt).toLocaleDateString('ar-SA-u-ca-gregory-nu-latn'),
        };
      })
    );
  ...
  const handleBulkSuspend = useCallback(async () => {
    try {
      ...
      toast.success(t('admin.subscribers.bulkSuspendSuccess').replace('{count}', selectedIds.length.toLocaleString('ar-SA-u-ca-gregory-nu-latn')));
      setRowSelection({});
      mutate();
    } catch {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-white/50 text-sm font-[family-name:var(--font-display)]">
            {t('admin.subscribers.totalCount').replace('{count}', filtered.length.toLocaleString('ar-SA-u-ca-gregory-nu-latn'))}
          </p>
        </div>
      ...
      {selectedIds.length > 0 && (
        <div className="bg-gold/10 border border-gold/20 rounded-xl p-4 flex items-center justify-between">
          <span className="text-gold font-[family-name:var(--font-display)] text-sm">
            {t('admin.subscribers.selected', { count: selectedIds.length.toLocaleString('ar-SA-u-ca-gregory-nu-latn') })}
          </span>
