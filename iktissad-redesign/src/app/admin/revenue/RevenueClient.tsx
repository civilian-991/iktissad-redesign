...
function formatMonth(month: string) {
  try {
    return new Date(month + '-01').toLocaleDateString('ar-SA-u-ca-gregory-nu-latn', {
      month: 'short',
      year: 'numeric',
    });
  } catch {
...
function ArabicTooltip({ active, payload, label, valueFormatter, labelPrefix }: TooltipProps) {
  ...
  const fmt = valueFormatter ?? ((v: number) => v.toLocaleString('ar-SA-u-ca-gregory-nu-latn'));
  return (
        ...
        {isLoading ? (
          ...
          <>
            ...
            <KpiCard
              ...
              value={(d?.churnRate ?? 0).toLocaleString('ar-SA-u-ca-gregory-nu-latn', { maximumFractionDigits: 1 }) + '%'}
              icon={TrendingDown}
            ...
            <KpiCard
              ...
              value={(d?.activeSubscribers ?? 0).toLocaleString('ar-SA-u-ca-gregory-nu-latn')}
              icon={Users}
            ...
            <KpiCard
              ...
              value={(d?.trialSubscribers ?? 0).toLocaleString('ar-SA-u-ca-gregory-nu-latn')}
              icon={Users}
            ...
            <KpiCard
              ...
              value={(d?.newThisMonth ?? 0).toLocaleString('ar-SA-u-ca-gregory-nu-latn')}
              icon={UserPlus}
              iconColor="bg-gradient-to-br from-gold to-bronze"
              trend="up"
              subLabel={`${(d?.canceledThisMonth ?? 0).toLocaleString('ar-SA-u-ca-gregory-nu-latn')} ملغى`}
            />
          </>
        )}
      </div>
            ...
            {(d?.planDistribution?.length ?? 0) === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value) => [
                      Number(value).toLocaleString('ar-SA-u-ca-gregory-nu-latn'),
                    ]}
                    contentStyle={{
                      background: OBSIDIAN,
