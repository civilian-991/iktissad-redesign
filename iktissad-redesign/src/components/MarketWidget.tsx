...
function formatPrice(value: number, currency: string, arabic = false): string {
  const locale = arabic ? 'ar-SA-u-ca-gregory-nu-latn' : 'en-US'
...
function formatDate(dateStr: string, arabic = false): string {
  return new Date(dateStr).toLocaleDateString(arabic ? 'ar-SA-u-ca-gregory-nu-latn' : 'en-US', { month: 'short', day: 'numeric' })
}
        ...
        <div className="flex flex-col items-end gap-0.5 mt-1 flex-shrink-0">
          <div className="text-xs text-white/30 flex items-center gap-1">
            ...
            <span>
              ...
              {new Date(data.lastUpdated).toLocaleTimeString(arabic ? 'ar-SA-u-ca-gregory-nu-latn' : 'en-US', { hour: '2-digit', minute: '2-digit' })}
            </span>
          </div>
          {data.isStale && (
